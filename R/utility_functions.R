...
#'@title Bootstrapping PCA sample
#'
...
#'standardized loadings using linear PCA (\emph{prcomp()}) or non-linear PCA (\emph{Gifi::princals()}).
#'
#'@author Abel Torres Espin
#'
...
#'@title Permutation PCA
#'
#'@description Generates a list of permutation samples from a PCA that can be used for
...
#'
#'@return Returns a list of objects in which each element is a permutated PCA solution of the
#'specified output.
#'
#'@references
#'\enumerate{
...
#'}
#'
permut_pca<-function(pca,pca_data, P=1000,ndim, output='VAF', pb=NULL, perm.method='permD'){
  ...
  permut_pca_V<-function(x, pb=NULL){
    ...
    for (i in 1:dim(x)[2]){
      ...
      if(class(pca)[1]=='prcomp'){
        ...
        pca_per<-prcomp(perm_x,scale. = .scale,center = center)
  ...
  if (output=='VAF' || perm.method=='permD'){
    ...
    per_samples<-replicate(P, permut_pca_V(pca_data, pb=pb), simplify = F)
  }
